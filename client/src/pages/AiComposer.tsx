...
import { ArrowRightIcon, Calendar1Icon, CalendarIcon, Clock1Icon, ClockIcon, HistoryIcon, Loader2, Loader2Icon, TimerIcon, Wand2Icon, XIcon } from "lucide-react";
        ...
        <div className="max-w-4xl  mx-auto space-y-12 pb-20 animate-in fade-in duration-700">
            ...
            <div className="space-y-6 text-center mt-20">
                ...
                <div className="relative group mt-12">
                    ...
                    <div className="absolute bottom-4 right-2.5 flex items-center gap-3 text-sm">
                        <button onClick={() => setGenerateImage(!generateImage)} className="flex items-center gap-3 bg-red-50 px-3 py-2 rounded-lg" type="button">
                            <span>AI Image</span>
            ...
            <div className="space-y-6 pt-12 border-t border-slate-100">
                <div className="flex items-center justify-between text-slate-600">
                    ...
                    <span className="text-sm text-slate-500 bg-slate-50 px-2">{generations.length}
                        total</span>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                    {generations.map((gen) => (
                        <div key={gen._id} className="group bg-white rounded-lg border border-slate-100 p-5 hover:border-red-200 transition-all relative overflow-hidden">
                            <div className="flex flex-col rounded-2xl h-full space-y-4">
                                <div className="flex items-center justify-between">
                                    <span className="text-xs text-slate-400 uppercase tracking-widest"> {new Date(gen.createdAt).toLocaleString()}</span>
                                    <span className="text-xs text-red-500 bg-red-50 px-2 py-0.5 rounded-lg">{gen.tone}</span>
                                </div>
                                <p className="line-clamp-3 text-sm text-slate-600 leading-relaxed flex-1">{gen.content}</p>
            ...
            {activeScheduler && (
                <div className="fixed inset-0 min-h-screen z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-md animate-in fade-in duration-300">
                    <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl border border-slate-100 overflow-hidden flex justify-between flex-col max-h-[90vh]">
                        <div className="flex items-center justify-between px-8 py-4 border-b border-slate-100 bg-slate50/30">
                        ...
                        <div className="p-8 bg-slate-50/50 border-t border-slate-50 space-y-8">
                            ...
                            <div className="space-y-6">
                                ...
                                {/* TIme and Date Selection Scheduler */}
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    ...
                                    <div className="relative">
                                        <Clock1Icon className="size-4 absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" />
                            ...
                            <button onClick={handleSchedule} className="w-full flex items-center justify-center gap-2 py-3 rounded-md bg-slate-200 text-slate-700 hover:bg-red-500 hover: text-white transition">
                                {loading ? <Loader2Icon className="size-4 animate-spin" /> : <TimerIcon className="size-4" />}
                                Schedule Post
                            </button>
                        </div>
                    </div>
