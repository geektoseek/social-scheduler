...
const Platformpickermodel = ({ connectedIds, connecting, onClose, onConnect }: PlatformpickermodelProps) => {
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur">
            <div className="bg-white rounded-2xl shodow-2xl w-full max-w-md border border-slate-100">
                {/* Header */}
                ...
                <div className="p-6 flex-col flex gap-2">
                    {PLATFORMS.map((p) => {
                        ...
                        return (
                            ...
                            >
                                <div className="p-2">
                                    <p.icon className={`size-5 ${isConnected ? "text-red-600" : "tex-slate-500"}`} />
                                </div>
                                <div className="flex-1 min-w-0">
                                    <div className={`text-sm ${isConnected ? "text-red-700" : "text-slate-800"}`}>
                                        {p.name}
                                    </div>
                                    <div className="text-sx text-slate-500 truncate">
                                        {isConnected ? "Already Connected" : p.description}
                                    </div>
                                </div>
                                {isConnected && <CheckCircleIcon className="size-4 text-red-500 shrink-0" />}
                                {isConnecting && <div className="size-4 border-2 border-red-600 border-t-transparent rouded-full animate-spin shrink-0" />}
