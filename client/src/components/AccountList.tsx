...
const AccountList = ({ accounts, onDisconnect }: AccountListProps) => {
    ...
    const handleDiconnect = async (accountId: string) => {
    ...
    if (accounts.length === 0) {
        return (
            <div className="bg-white rounded-2xl border-2 border-dashed border-slate-200 flex flex-col items-center justify-center py-20 px-6">
                ...
                <p className="text-sm text-slate-400 mt-1 max-xs-xs text-center">Connect your first social platform to start scheduling and automating your content.</p>
            </div>
        )
    }
    ...
    return (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {accounts.map((account, index) => {
                ...
                if (!meta) return null;
                return (
                    <div key={index} className="group bg-white border border-slate-200 rounded-2xl flex items-center gap-4 hover:border-slate-300 transition-all py-3">
                        ...
                        <div className="flex items-center gap-1.5 shrink-0">
                            {account.status === 'connected' ? (
                                <>
                                    ...
                                    <span className="tex-sm text-emerald-600">Connected</span>
                                </>
                            ) : (
                                <>
                                    <AlertCircleIcon className="size-4 text-amber-500" />
                                    <span className="text-sm text-amber-600">Diconnected</span>
                                </>
                            )}
                        </div>
                        <button className="ml-2 p-1.5 rounded-lg text-slate-300 group-hover:text-red-500 transition-all"
                            onClick={() => handleDiconnect(account._id)}
                            title="Diconnect account"
                        >
